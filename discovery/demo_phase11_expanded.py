...
def main():
    print("=== CoAI Phase 11 Expanded: 30-Cycle Discovery (Balanced Profile) ===")
    
    # Force unbuffered output for background logging
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(line_buffering=True)
    
    print("Initializing explorer...", flush=True)
    ...
    print(f"\nExpanded run completed in {duration:.2f} seconds.")
    ...
    results = {
        ...
        "total_cycles": session.stats.get("cycles", 30),
    ...
    out_path = Path("docs/phase11_expanded_results.json")
